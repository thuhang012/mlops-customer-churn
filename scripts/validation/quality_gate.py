...
def check_quality_gate(
    current_metrics_path: str, baseline_metrics_path: str = "artifacts/baseline/metrics.json", threshold: float = 0.95
) -> bool:
    """
    ...
    try:
        ...
        if baseline_metrics_path.exists():
            ...
            baseline = {"f1_score": 0.60, "roc_auc": 0.65}
        ...
        print(f"F1 Score:    {current_f1:.4f} >= {threshold_f1:.4f}? {'PASS' if f1_pass else 'FAIL'}")
        if not f1_pass:
            f1_degradation_pct = ((baseline_f1 - current_f1) / baseline_f1) * 100
            max_degradation_pct = (1 - threshold) * 100
            print(
                f"  Degradation: {f1_degradation_pct:.1f}% "
                f"(max: {max_degradation_pct:.1f}%)"
            )
        ...
        print(f"ROC AUC:     {current_auc:.4f} >= {threshold_auc:.4f}? {'PASS' if auc_pass else 'FAIL'}")
        if not auc_pass:
            auc_degradation_pct = ((baseline_auc - current_auc) / baseline_auc) * 100
            max_degradation_pct = (1 - threshold) * 100
            print(
                f"  Degradation: {auc_degradation_pct:.1f}% "
                f"(max: {max_degradation_pct:.1f}%)"
            )
        ...
        if passed:
            print("\nQuality Gate PASSED")
            print("Current metrics are acceptable for deployment")
        else:
            print("\nQuality Gate FAILED")
            print("Model degradation exceeds threshold")
